...
REMOTE_ACTION="$(space /_remote_plumbing/action/ -e SPACE_MUTE_EXIT_MESSAGE=1 -d)"
...
REMOTE_POD_INFO="$(space /_remote_plumbing/pod_info/ -e SPACE_MUTE_EXIT_MESSAGE=1 -d)"
...
export REMOTE_PACK_RELEASEDATA REMOTE_SET_COMMITCHAIN REMOTE_ACQUIRE_LOCK REMOTE_RELEASE_LOCK REMOTE_GET_HOSTMETADATA REMOTE_UPLOAD_ARCHIVE REMOTE_UNPACK_ARCHIVE REMOTE_INIT_HOST REMOTE_HOST_SETUP REMOTE_LOGS REMOTE_DAEMON_LOG REMOTE_CREATE_SUPERUSER REMOTE_DISABLE_ROOT REMOTE_ACTION REMOTE_SIGNAL REMOTE_POD_STATUS REMOTE_POD_INFO REMOTE_POD_SHELL REMOTE_HOST_SHELL
space /_cmdline/ -e SPACE_MUTE_EXIT_MESSAGE=1 -d >./release/sns
chmod +x ./release/sns
